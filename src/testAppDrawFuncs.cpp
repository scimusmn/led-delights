...
void testApp::bottomBarDraw()
{
	...
	ofShade(0, ofGetHeight()-bottomBarH, 20, ofGetWidth(), OF_DOWN,.25);
...
//	ofShade(ofGetWidth()-rightBarWidth, ofGetHeight()-bottomBarH, 5, bottomBarH, OF_LEFT,.5);
	...
	ofRoundShadow(curBox.x-20, curBox.y-20, curBox.width+40, curBox.height+20, 10, .5);
...
void testApp::uploadDraw(){
	...
	ofRoundBox(upXoff, upYoff, ofGetWidth()-2*upXoff, ofGetHeight()-upYoff*2, 20);
	...
	ofSetColor(64, 64, 64);
	ofRoundBox(tabs.x+50, tabs.y+50, tabs.w-100, tabs.h-100, 20, 0);
	//ofSetColor(0, 0, 0);
